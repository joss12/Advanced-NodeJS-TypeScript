...
const platform_exceptions_1 = require("../../platform-exceptions");
...
class PlatformBuilder {
    ...
    async runLifecycle() {
        ...
        const routes = await utils_1.importProviders(this.injector);
    ...
    async loadInjector() {
        ...
        await utils_1.loadInjector(injector, utils_1.createContainer(core_1.constructorOf(this.rootModule)));
    ...
    async createContext() {
        const middleware = new platform_1.PlatformContextMiddleware(this.injector);
    ...
    async loadRoutes(routes) {
        ...
        if (this.settings.logger.level !== "off") {
            if (core_1.classOf(this.injector.get(platform_1.LogIncomingRequestMiddleware)) !== platform_1.LogIncomingRequestMiddleware) {
                this.app.use(platform_1.LogIncomingRequestMiddleware);
            }
            else {
        ...
        this.app.use(platform_exceptions_1.PlatformExceptionsMiddleware);
        this.app.use(platform_exceptions_1.GlobalErrorHandlerMiddleware);
    }
