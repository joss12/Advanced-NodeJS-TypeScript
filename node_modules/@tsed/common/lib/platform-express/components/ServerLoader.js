...
const LogIncomingRequestMiddleware_1 = require("../../platform/middlewares/LogIncomingRequestMiddleware");
...
class ServerLoader extends platform_builder_1.PlatformBuilder {
    ...
    async loadSettingsAndInjector() {
        ...
        const routes = await platform_builder_1.importProviders(this.injector);
    ...
    async loadRoutes(routes) {
        this.settings.logger.level !== "off" && this.app.use(LogIncomingRequestMiddleware_1.LogIncomingRequestMiddleware);
        await this.callHook("$onMountingMiddlewares"); // deprecated
        await super.loadRoutes(routes);
        // Import the globalErrorHandler
        this.use(platform_exceptions_1.PlatformExceptionsMiddleware);
        this.use(platform_exceptions_1.GlobalErrorHandlerMiddleware);
    }
    /* istanbul ignore next */
...
ServerLoader.isServerLoader = true;
tslib_1.__decorate([
